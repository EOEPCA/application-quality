...
- Use <img src="../img/user-manual/mdi-animation-play-outline-blue.png" style="height:20px; width:20px"/> to display the details of the external event that triggered the pipeline execution, if any (see below, and further information in [Analysis Pipeline Triggers](automation.md#analysis-pipeline-triggers)).
- Use <img src="../img/user-manual/mdi-note-text-outline-blue.png" style="height:20px; width:20px"/> to navigate to the **Execution Reports** page and inspect the report generated by each tool integrated in the pipeline (see [Execution Reports](#inspection-of-the-execution-reports), below). The badge  displayed on the icon shows the number of generated reports.
...
- The 3-dot <img src="../img/user-manual/mdi-dots-vertical-black.png" style="height:20px; width:20px"/> menu icon reveals a menu that allows overriding the quality status manually. The new value is propagated to GitHub if the pipeline has been triggered by a GitHub event (see [Manually Setting the Quality Status](automation.md#manually-setting-the-quality-status)). The table then displays the original status and the manually set status (e.g. "Fail -> Pass").
...
An overview of the digest values (number of warnings, errors, etc.) is included in the table.
