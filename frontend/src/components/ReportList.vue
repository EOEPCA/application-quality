<template>
    <v-card flat>
      ...
      <template v-slot:item="{item, props}">
        <v-list-item v-bind="props" :title="formatDate(item.props.title)"></v-list-item>
      </template>
    ...
    <v-data-table
      ...
      >
        ...
        <template v-slot:item="{ item }">
          <tr>
            <td>{{ store.pipelineById(store.executionById(item.run).pipeline).description || '-' }}</td>
            <td>{{ store.pipelineById(store.executionById(item.run).pipeline).version || '-' }}</td>
        ...
        <v-card v-if="selectedReport">
          ...
          <v-card-text>
            <v-alert
              ...
              class="mb-4"
            />
            <pre class="report-json">{{ JSON.stringify(selectedReport, null, 2) }}</pre>
          </v-card-text>
        </v-card>
      </v-dialog>
    ...
    methods: {
      ...
      async refreshReports() {
        ...
        if (this.store.selectedExecutionId) {
          console.info("Retrieving reports for pipeline execution", this.store.selectedPipelineId, this.store.selectedExecutionId)
          await this.store.fetchPipelineExecutionReports(this.store.selectedPipelineId, this.store.selectedExecutionId)
        } else {
